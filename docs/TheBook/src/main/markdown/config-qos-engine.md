...
1. the ``QoS Verifier``:  This used to be the heart of the Resilience service.
   ...
   The operations continue to be maintained in memory as before, and
...
memory requirements for the verification component are fairly substantial, particularly
for large dCache installations.
...
Whenever feasible, it is recommended to give QoS its own domain(s).
...
the file is not on disk already.
...
to only one such preferential group (an exception will occur when violation of this rule
...
#### verifier-operation-map (qos-verifier)

This is the snapshot of the main verification operation table. By default,
this checkpoint is written out (in simple text format) every minute. This
...
all verify operations in progress at that time may have actually been saved, nor
completed ones removed. The periodic pool scans (see below) should
eventually detect any missing replicas not captured at restart.

When a new snapshot is taken, the old is first renamed to
`verifier-operation-map-old`. Similarly, at restart, before being reread, the file
is renamed to `verifier-operation-map-reload` to distinguish it from the current
...
pool cancel [OPTIONS] [regular expression]  # cancel pool operations
pool ctrl [OPTIONS] [on|off|start|shutdown|reset|run|info]  # control the periodic check of active pools or processing of pool state changes
...
verify cancel [OPTIONS] [pnfsids]  # cancel file operations
verify ctrl [OPTIONS] [on|off|start|shutdown|reset|run|info]  # control checkpointing or handling of operations
...
One of the most useful commands is `info`, for diagnostics.  Here it is from
the singleton deployment, where everything runs as a single cell (this would
be distributed over the four components were they running separately, but
could still be viewed all at once by doing ``\s qos-* info``):

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
[fndcatemp2] (qos@qosDomain) admin > info
--- AdjusterTaskMap ---
Running since: Tue Nov 17 11:25:25 CST 2020
Uptime 0 days, 1 hours, 5 minutes, 2 seconds

Last sweep at Tue Nov 17 12:30:25 CST 2020
...
maximum staging allocation 0.5
maximum retries on failure 1

sweep interval 1 MINUTES
checkpoint interval 1 MINUTES
checkpoint file path /var/lib/dcache/qos/verifier-operation-map
task launch delay 0
...
Last checkpoint at Tue Nov 17 12:29:55 CST 2020
Last checkpoint took 0 seconds
Last checkpoint saved 0 records
...
CORRUPT_FILE                          0
QOS_ACTION_COMPLETED                  0
QOS_MODIFIED                          0
QOS_MODIFIED_CANCELED                 0
QOS_REQUIREMENTS_REQUEST              0
...
Last sweep at Tue Nov 17 12:28:27 CST 2020
Last sweep took 180 seconds

ACTION                       COMPLETED       FAILED
POOL_STATUS_DOWN                     0            0
POOL_STATUS_UP                      41            0                0               0           0
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
CHECK_CUSTODIAL_ONLINE	(completed: 0 / 0 : ?%) – (updated: 2020/11/17 11:28:27)(scanned: 2020/11/17 11:28:27)(prev ENABLED)(curr ENABLED)(IDLE)
dcatest03-1	(completed: 0 / 0 : ?%) – (updated: 2020/11/17 11:28:27)(scanned: 2020/11/17 11:28:27)(prev ENABLED)(curr ENABLED)(IDLE)
dcatest03-2	(completed: 0 / 0 : ?%) – (updated: 2020/11/17 11:28:27)(scanned: 2020/11/17 11:28:27)(prev ENABLED)(curr ENABLED)(IDLE)
dcatest03-3	(completed: 0 / 0 : ?%) – (updated: 2020/11/17 11:28:27)(scanned: 2020/11/17 11:28:27)(prev ENABLED)(curr ENABLED)(IDLE)
dcatest03-4	(completed: 0 / 0 : ?%) – (updated: 2020/11/17 11:28:27)(scanned: 2020/11/17 11:28:27)(prev ENABLED)(curr ENABLED)(IDLE)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
CHECK_CUSTODIAL_ONLINE    |            1            0 |            1            0            1 |            0               152116
...
Finally, the three `ctrl` (verify, task, pool) commands can be used to verify
or reset basic configuration values, or to interrupt operation processing or
force a sweep to run.

## Tuning

Only a few properties can be reset using the `ctrl` commands mentioned above. Please
...
The special pool placeholder you see in the ``pool ls`` output, ``CHECK_CUSTODIAL_ONLINE``,
stands for a query which runs after a periodic scan of the rest of
the pools takes place.  It checks for `CUSTODIAL ONLINE` files which may have
somehow lost their disk copy (that is, with no disk locations registered in
the namespace; such orphaned files would not be caught during the normal
pool scans because those check files with a particular disk location).  If
any such files are discovered, QoS also attempts to stage them back.
...
>   another set of requests and handles them accordingly.**
...
NOTE:   As long as at least one replica of a file continues to reside in a primary
        ...
        (2b)    If a file has replicas only on pools which have been removed from
                ...
                files will become invisible to dCache.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
\s qos-scanner pool ctrl reset -down=1 -unit=MINUTES
down grace period 1 MINUTES
restart grace period 6 HOURS
maximum concurrent operations 5
scan window set to 24 HOURS
...
\s qos-scanner pool ctrl reset -restart=1 -unit=MINUTES
down grace period 1 MINUTES
restart grace period 1 MINUTES
maximum concurrent operations 5
scan window set to 24 HOURS
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\s qos-verifier verify ctrl shutdown
\s qos-verifier verify ctrl start
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

but the bug may present itself again.

The above `verify ctrl` command is nevertheless useful for attempting recovery
without stopping the domain; one can similarly start and stop the pool operation
and adjuster task consumer using `pool ctrl` and `task ctrl`.
